...
\begin{onecolentry}
    \begin{highlights}
        \item \textbf{Kỹ sư Hệ thống đa ngành \& Lập trình viên Backend}, kết hợp chuyên môn sâu về hạ tầng doanh nghiệp (IDC, Ảo hóa) với kiến trúc phần mềm hiệu năng cao.
        \item \textbf{Hạ tầng (Infrastructure):} Máy chủ \& Lưu trữ (SAN/NAS, Dell/HPE), Ảo hóa (VMware/Proxmox, K8s), Microsoft Core (AD DS), và Bảo mật Trung tâm Dữ liệu (Firewalls, WAF).
        \item \textbf{Phần mềm \& DevOps:} Backend (Golang, Python, gRPC, DDD), CI/CD Pipelines, SRE, và Tự động hóa Hạ tầng.
    \end{highlights}
\end{onecolentry}
...
\begin{twocolentry}{\textit{Tháng 4 2025 -- Hiện tại}}
    ...
    \textit{Kỹ sư Hệ thống \& Lập trình viên Phần mềm}
\end{twocolentry}
\begin{onecolentry}
    \begin{highlights}
...
\begin{onecolentry}
    \begin{highlights}
        \item Xây dựng nền tảng xử lý đơn hàng (Golang, gRPC, DDD) tích hợp Shopee, Lazada, TikTok với cơ chế quản lý phiên bản tồn kho và khóa phân tán (distributed locking).
        \item Thiết kế và vận hành hạ tầng IDC on-premise (Proxmox VE, Kubernetes, lưu trữ NAS, mạng) với cơ chế failover HA.
        \item Triển khai chiến lược lưu trữ phân tầng hot/warm/cold trên hệ thống NAS để quản lý vòng đời dữ liệu.
        ...
        \item Xây dựng hạ tầng giám sát SRE và các pipeline CI/CD (GitHub Actions, Drone CI, Kubernetes).
...
\begin{onecolentry}
    \begin{highlights}
        ...
        \item Thiết kế và triển khai pipeline dữ liệu thời gian thực hướng sự kiện (event-driven) bằng AWS EventBridge để đồng bộ giữa Supabase và các hệ thống cũ.
...
\begin{onecolentry}
    \begin{highlights}
        ...
        \item Tối ưu hóa truy vấn MongoDB/MySQL với các chiến lược lập chỉ mục và phân tích truy vấn (query profiling), giúp giảm 40\% thời gian phản hồi.
